...
`OuDiaSecond.1.01` 形式は、 `OuPropertiesText`記法を用いてダイヤグラムの各要素をシリアル化します。

## `OuDiaSecond.1.00` 形式からの変更点

- `/FileType` が `OuDiaSecond.1.00` から `OuDiaSecond.1.01` に変更されました。(OuDiaSecond v1.02より)
- 駅番線に関する項目が追加されました。
	- `EkiTrack2Cont` Directory が追加されました。
	- `EkiTrack2` Directory が追加されました。
- `Eki` Directoryに `DownMain` / `UpMain` Property が追加されました。
- `Eki` Directoryに `BrunchCoreEkiIndex` / `LoopOriginEkiIndex` Property が追加されました。
- `Eki` Directoryに `JikokuhyouTrackDisplayKudari` / `JikokuhyouTrackDisplayNobori` Property が追加されました。
- `Eki` Directoryに `DiagramTrackDisplay` Property が追加されました。
- `JikokuhyouBackColor` Property が追加されました。
- `RessyaTrack` Property が追加されました。
	- OuDiaSecond V1.02.03で入換着時刻について追加されています。
- `MainBackColorIndex` / `SubBackColorIndex` / `BackPatternIndex` Property が追加されました。
- `JikokuhyouBackColor` Property が追加されました。
- `StdOpeTimeLowerColor` / `StdOpeTimeHigherColor` / `StdOpeTimeUndefColor` / `StdOpeTimeIllegalColor` Property が追加されました。
...
|*DownMain*|Unsigned Int (Property)|Optional|下り本線の番線インデックスを表します。|
|*UpMain*|Unsigned Int (Property)|Optional|上り本線の番線インデックスを表します。|
|*BrunchCoreEkiIndex*|Unsigned Int(Property)|Optional|分岐駅のインデックスを表します。|
|*LoopOriginEkiIndex*|Unsigned Int(Property)|Optional|環状設定駅のインデックスを表します。|
|*JikokuhyouTrackDisplayKudari*|Boolean (Property)|Optional|下り時刻表上において番線を表示するかどうかを表します。|
|*JikokuhyouTrackDisplayNobori*|Boolean (Property)|Optional|上り時刻表上において番線を表示するかどうかを表します。|
|*DiagramTrackDisplay*|Boolean (Property)|Optional|ダイヤグラム上に番線毎の在線表を表示するかどうかを表します。|
|*EkiTrack2Cont*|[EkiTrack2Cont](#ekitrack2cont-directory) (Directory)|Optional|駅に所属する番線一覧を表します。|
...
## EkiTrack2Cont Directory

子要素として複数の `EkiTrack2` Directory を含むディレクトリです。

## EkiTrack2 Directory

|Key|Type|Required|概要|
|---|---|---|---|
|TrackName|String (Property)|Required|番線名を表します。この項目は必須です。空文字の場合エラーとなります。|
|TrackRyakusyou|String (Property)|Required|番線名の略称を表します。この項目は必須です。空文字の場合エラーとなります。|
...
|*JikokuhyouBackColor*|[Color](#color-property)|Optional|時刻表の背景色を表します。|
...
|*MainBackColorIndex*|Unsigned Int (Property)|Optional|基本背景色の時刻表背景色インデックスを表します。|
|*SubBackColorIndex*|Unsigned Int (Property)|Optional|補助背景色の時刻表背景色インデックスを表します。|
|*BackPatternIndex*|Unsigned Int (Property)|Optional|背景色のパターンを表します。|
...
|*RessyaTrack*|[RessyaTrack](#ressyatrack-property) (Property)|Optional|列車の発着番線一覧を表します。|
...
### RessyaTrack Property

列車に含まれる駅時刻の番線リストを表します。

`RessyaTrack` Propertyはカンマ区切りの1次元配列の値として表されます。

以下のいずれかの形式を用いて発着番線を指定します。

- `{unsigned integer}` の場合 発着する番線のインデックスのみ指定
- `{unsigned integer};{unsigned integer}` の場合、発着する番線のインデックス、始終着駅の作業情報を指定
- `{unsigned integer};{unsigned integer}/{string}` の場合、発着する番線のインデックス、始終着駅の作業情報、作業情報の引数を指定
- `{unsigned integer};{unsigned integer}/{string}${jikoku}` の場合、発着する番線のインデックス、始終着駅の作業情報、作業情報の引数、入換発時刻を指定
- `{unsigned integer};{unsigned integer}/{string}${jikoku}/{jikoku}` の場合、発着する番線のインデックス、始終着駅の作業情報、作業情報の引数、入換発時刻、入換着時刻を指定
...
|*StdOpeTimeIllegalColor*|[Color](#color-property)|Optional|基準運転時分が不正のときの時刻表上の背景色を表します。|
